...
import { CommonButton, CommonInput } from '../../styles/common';
import { ICategoryGroup } from '../../utils/gql-client/props';
...
`;
const SelectionButton = styled(CommonButton)``;

export const SelectionPanelButtonGroup: FC<{
  ...
  return (
    <SelectionButtonGroup>
      {enableSubmitButton && (
        <SelectionButton className="SubmitButton" onClick={submitHandler}>
          确认
        </SelectionButton>
      )}
      {enableCancelButton && (
        <SelectionButton className="CancelButton" onClick={cancelHandler}>
          取消
        </SelectionButton>
      )}
    </SelectionButtonGroup>
...
const KeywordsSelectionPanelWrapper = styled.div`
  margin: 5px;
`;
...
export const KeywordsSelectionPanel: FC<{
  currentValue: string;
  ...
  const [keyword, setKeyword] = useState<string>(currentValue);
...
const CategorySelectionPanelComp: FC<{
  currentValue: ICategoryGroup;
  submitHandler: (selected: ICategoryGroup) => void;
  cancelHandler: () => void;
}> = ({
  currentValue = {},
  submitHandler = void 0,
  ...
  const categoryDataProcessor = (data: {
    ...
    let validCategoryItems = validCategories.map((item, index) => {
      return {
        key: index + 1,
        value: item,
      };
...
const AmountRangeSelectionPanelComp = ({
  currentValue = [],
  submitHandler = () => void 0,
  cancelHandler = () => void 0,
  dispatch,
  ...
  const submitHandlerFn = () => {
    for (const item in amountRanges) {
      if (
        amountRanges[item].minAmount > amountRanges[item].maxAmount &&
        amountRanges[item].maxAmount != 0
      ) {
        dispatch({
          type: 'SET_MESSAGE',
  ...
  return (
    <div className="AmountRangeSelectionPanel">
      <div className="AmountRangeAdder" onClick={amountRangeAdder}>
        +
      </div>
        ...
        {amountRanges.map(({ minAmount, maxAmount }, index) => (
          <div className="AmountRangeItem" key={index}>
            <div className="AmountRangeMain">
              <input
                className="AmountRangeItem--Min"
                onChange={evt => onMinAmountChange(evt, index)}
                value={minAmount}
              />
              <div className="AmountRangeItem--Sep"> - </div>
              <input
                className="AmountRangeItem--Max"
                onChange={evt => onMaxAmountChange(evt, index)}
                value={maxAmount}
              />
            </div>
            <div
              className="AmountRangeRemover"
              onClick={() => {
                amountRangeRemover(index);
              }}
            >
              -
            </div>
...
export const AmountRangeSelectionPanel = connect()(
  AmountRangeSelectionPanelComp
);

export const DateRangeSelectionPanel = ({
  currentValue = {},
  submitHandler = () => void 0,
  cancelHandler = () => void 0,
}) => {
  const [selectedDateRange, setSelectedDateRange] = useState(currentValue);
  const [isFreeDateRangeEnabled, setIsFreeDateRangeEnabled] = useState(
    currentValue.key === 9
  );
  const [customStartDate, setCustomStartDate] = useState(
    currentValue.key === 9 ? new Date(currentValue.minDate) : new Date()
  );
  const [startDateStatus, setStartDateStatus] = useState(false);
  const [customEndDate, setCustomEndDate] = useState(
    currentValue.key === 9 ? new Date(currentValue.maxDate) : new Date()
  );
    ...
    if (date < day) {
      ...
      return {
        minDate: formatDate(
          year,
          month - 1,
          daysInMonth(year, month - 1) - (diff - 1)
        ),
    ...
    if (date - 7 <= 0) {
      return {
        ...getThisWeek(
          year,
          month - 1,
          daysInMonth(year, month - 1) - (7 - date),
          day
        ),
      };
    } else {
        ...
        <div className="DateRangeSelection--Custom">
          <div className="DateRangeSelection DateRangeSelection--CustomStart">
            <div
              ...
              onClick={toggleStartDateSelection}
            >
...
export const CitySelectionPanel = ({
  currentValue = ALL_KEYWORD,
  submitHandler = () => void 0,
  cancelHandler = () => void 0,
}) => {
  ...
  useEffect(() => {
    Axios.get('/api/wacai/getAllCities').then(({ data }) => {
...
export const TagSelectionPanel = ({
  currentValue = ALL_KEYWORD,
  submitHandler = () => void 0,
  cancelHandler = () => void 0,
}) => {
