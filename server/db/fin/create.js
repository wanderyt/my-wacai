...
const createFinItem = (db, data, callback) => {
  let promise = new Promise((resolve, reject) => {
    let sql = `insert into ${FIN_TABLE_NAME}(id, category, subcategory, date, comment, details, amount, place, city, userid, tags)
      values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);`;
    let searchParams = [
      data.id,
      data.category,
    ...
    db.all(sql, searchParams, (err) => {
      if (err) {
        logDBError(
          `createFinItem - Create fin data in ${FIN_TABLE_NAME} table with params: ${searchParams}`,
          sql,
          err
        );
      } else {
...
const formatSchedule = (sqlTemplate, datetime, scheduleMode) => {
  ...
  for (let index = 0; index <= loopTimes; index++) {
    ...
    sqls.push(
      sqlTemplate
        .replace("{{datetimeFn}}", datetimeSql)
        .replace("{{id}}", id)
        .replace("{{scheduleId}}", currentScheduleId)
    );
  }
...
const createScheduledFinItem = (db, data, callback) => {
  let promise = new Promise((resolve, reject) => {
    ...
    let insertHeaderSQL = `insert into ${FIN_TABLE_NAME}(id, category, subcategory, date, comment, details, amount, isScheduled, scheduleId, place, city, userId, tags) `;
    let insertRowSQL = `select "{{id}}", "${data.category}", "${
      data.subcategory
    }", {{datetimeFn}}, "${data.comment || ""}", "${data.details || ""}", ${
      data.amount
    }, ${scheduleMode}, '{{scheduleId}}', "${data.place || ""}", "${
      data.city || ""
    }", ${data.userId || 1}, "${data.tags || ""}"`;
    let sqlList = formatSchedule(
      insertHeaderSQL + insertRowSQL + ";",
      data.date,
      scheduleMode
    );
    db.exec(sqlList.join(""), (err) => {
      if (err) {
        logDBError(
          `createScheduledFinItem - Create scheduled fin data in ${FIN_TABLE_NAME} table`,
          JSON.stringify(data),
          err
        );
      } else {
 ...
 * @param {string} data.details Target new template details data.
...
const createFinTemplate = (db, data, callback) => {
  let promise = new Promise((resolve, reject) => {
    if (data) {
      let { category, subcategory, comment, details, place, userId } = data;
      let querySQL = `select * from ${TEMPLATE_TABLE_NAME} where category = ? and subcategory = ? and comment = ? and details = ? and place = ? and userId = ?;`;
      let searchParams = [
        category,
        subcategory,
        comment,
      ...
      db.all(querySQL, searchParams, (err, rows) => {
        if (err) {
          logDBError(
            `Query if target template is already existed`,
            querySQL,
            err
          );
        } else {
          ...
          if (rows.length > 0) {
            ...
            db.run(insertSQL, searchParams, (err) => {
              if (err) {
                logDBError(
                  `Insert new template in template table`,
                  insertSQL,
                  err
                );
              } else {
